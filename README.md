...
# Usage
Follow the next steps in order to generate property content trends for pretrained models from raw wavforms and alignments. Each step is accompanied by a short explanation. You can also find an abridged and collated version of these steps at [examples/recipe.sh](https://github.com/ankitapasad/layerwise-analysis/blob/main/examples/recipe.sh). You can run the script as `. examples/recipe.sh` after performing steps 0, 1, and 2a below. Read the accompanying [README.md](https://github.com/ankitapasad/layerwise-analysis/blob/main/examples/README.md) with the recipe script before you run it.
...
```
This will download and reformat the phone and word alignment files for Librispeech and save the alignments into dictionary files to `$alignment_data_dir`. These `.pkl` files map each phone/word type to a list of tuples `(utt_id, path_to_wav, start_time, end_time)`.
